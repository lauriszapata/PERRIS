"""
Backtest OPTIMIZADO 30 días
Con cambios propuestos para mejorar rentabilidad
"""
...
OPTIMIZED_CONFIG = {
    # Trading Hours: 5 AM - 11:50 AM (vs 5 AM - 2 PM)
    'TRADING_START_HOUR': 5,
    'TRADING_END_HOUR': 11,
    'TRADING_END_MINUTE': 50,
    
    # Stop Loss: 3.5 ATR (vs 2.5)
    'SL_ATR_MULTIPLIER': 3.5,
    'TRAILING_SL_ATR_MULTIPLIER': 2.0,  # vs 1.5
    
    # Exposure: 150 USD (vs 125)
    'FIXED_EXPOSURE_USD': 150,
    
    # Take Profit: Simplified (less partials = less commissions)
    'TP_LEVELS': [
        {"pct": 0.020, "close_pct": 0.30, "name": "P1"},  # 2.0% → 30%
        {"pct": 0.035, "close_pct": 0.30, "name": "P2"},  # 3.5% → 30%
    ],
    'DYNAMIC_SCALPING_START': 0.050,  # Start at 5% vs 2.5%
    'DYNAMIC_SCALPING_INCREMENT': 0.010,  # 1% vs 0.5%
    'DYNAMIC_SCALPING_CLOSE_PCT': 0.10,  # 10% vs 5%
    
    # Filters: More strict
    'ADX_MIN': 25,  # vs 20
    'VOLUME_MIN_MULTIPLIER': 1.3,  # vs 1.0
    'VOLATILITY_MAX': 0.025,  # vs 0.03
}

# Blacklist symbols (consistent losers)
SYMBOL_BLACKLIST = [
    "POL/USDT", "NEAR/USDT", "APT/USDT", "TRX/USDT", "LINK/USDT"
]

# Filter symbols
OPTIMIZED_SYMBOLS = [s for s in Config.SYMBOLS if s not in SYMBOL_BLACKLIST]


class ATRManagerOptimized:
    @staticmethod
    def calculate_initial_stop(entry_price, atr_entry, direction):
        multiplier = OPTIMIZED_CONFIG['SL_ATR_MULTIPLIER']
        if direction == "LONG":
    ...
    def calculate_trailing_stop(current_sl, extreme_price, current_atr, direction, entry_price):
        multiplier = OPTIMIZED_CONFIG['TRAILING_SL_ATR_MULTIPLIER']
        if direction == "LONG":
...
class EntrySignalsOptimized:
    ...
    def check_signals(df, direction):
        ...
        try:
            ...
            # STRICTER ADX: >= 25 (vs 20)
            adx_val = last['ADX']
            results['ADX'] = {'status': adx_val >= OPTIMIZED_CONFIG['ADX_MIN'], 'value': f"{adx_val:.2f}"}
            
            # Same RSI
            rsi_val = last['RSI']
            if direction == "LONG":
                results['RSI'] = {'status': rsi_val > 35, 'value': f"{rsi_val:.2f}"}
            else:
                results['RSI'] = {'status': 30 < rsi_val < 55, 'value': f"{rsi_val:.2f}"}
            
            # Same MACD
            macd_line = last['MACD_line']
            macd_signal = last['MACD_signal']
            if direction == "LONG":
                results['MACD'] = {'status': macd_line > macd_signal, 'value': f"L:{macd_line:.4f}/S:{macd_signal:.4f}"}
            else:
                results['MACD'] = {'status': macd_line < macd_signal, 'value': f"L:{macd_line:.4f}/S:{macd_signal:.4f}"}
            
            # STRICTER Volume: >= 1.3x (vs 1.0x)
            vol = last['volume']
            vol_avg = last['Vol_SMA20']
            vol_min = OPTIMIZED_CONFIG['VOLUME_MIN_MULTIPLIER']
            results['Volume'] = {'status': vol >= vol_min * vol_avg, 'value': f"{vol:.2f}"}
            
            # STRICTER Volatility: < 2.5% (vs 3%)
            atr_val = last['ATR']
            close_val = last['close']
            volatility_pct = atr_val / close_val
            vol_max = OPTIMIZED_CONFIG['VOLATILITY_MAX']
            results['Volatility'] = {'status': volatility_pct < vol_max, 'value': f"{volatility_pct:.2%}"}
            
            # MTF and Structure (optional)
            results['MTF_Trend'] = {'status': True, 'value': 'Skipped', 'optional': True}
            results['Structure'] = {'status': True, 'value': 'Optional', 'optional': True}
            
            # Decision
            standard_entry = all(r['status'] for k, r in results.items() if not r.get('optional', False))
            
            # Early Entry (same logic)
...
class OptimizedBacktester:
    def __init__(self, initial_balance=10000):
        ...
        self.max_open_symbols = Config.MAX_OPEN_SYMBOLS
        self.cooldown_minutes = Config.SYMBOL_COOLDOWN_MINUTES
        self.fixed_exposure_usd = OPTIMIZED_CONFIG['FIXED_EXPOSURE_USD']
    ...
    def is_trading_hours(self, timestamp):
        ...
        if hour < OPTIMIZED_CONFIG['TRADING_START_HOUR']:
            return False
        if hour > OPTIMIZED_CONFIG['TRADING_END_HOUR']:
            return False
        if hour == OPTIMIZED_CONFIG['TRADING_END_HOUR'] and minute >= OPTIMIZED_CONFIG['TRADING_END_MINUTE']:
            return False
    ...
    def run_backtest(self, data_map, start_date, end_date):
        print(f"\n{'='*80}")
        print(f"🔬 BACKTEST OPTIMIZADO: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        print(f"{'='*80}")
        print(f"Horario: 5:00 AM - 11:50 AM (ESTRICTO)")
        print(f"SL: {OPTIMIZED_CONFIG['SL_ATR_MULTIPLIER']}x ATR (vs 2.5x)")
        print(f"Exposure: ${OPTIMIZED_CONFIG['FIXED_EXPOSURE_USD']} (vs $125)")
        print(f"TP Levels: 2 fijos + dinámicos cada 1% (vs 4 + cada 0.5%)")
        print(f"Filtros: ADX≥25, Vol≥1.3x, Volatility<2.5% (MÁS ESTRICTOS)")
        print(f"Símbolos: {len(OPTIMIZED_SYMBOLS)} (excluye 5 perdedores)")
        print(f"{'='*80}\n")
        ...
        for symbol in list(self.open_positions.keys()):
            ...
            if pos['direction'] == 'LONG':
                if current_high > pos['highest_price']:
                    ...
                    new_sl = ATRManagerOptimized.calculate_trailing_stop(
    ...
    def _check_tp(self, symbol, pos, current_price, current_high, current_low, current_time):
        ...
        # Fixed levels
        for level in OPTIMIZED_CONFIG['TP_LEVELS']:
        ...
        # Dynamic levels
        if pnl_pct >= OPTIMIZED_CONFIG['DYNAMIC_SCALPING_START']:
            dynamic_levels = int((pnl_pct - OPTIMIZED_CONFIG['DYNAMIC_SCALPING_START']) / OPTIMIZED_CONFIG['DYNAMIC_SCALPING_INCREMENT'])
            
            for n in range(dynamic_levels + 1):
                level_pct = OPTIMIZED_CONFIG['DYNAMIC_SCALPING_START'] + (n * OPTIMIZED_CONFIG['DYNAMIC_SCALPING_INCREMENT'])
                level_key = f"D{n+1}"
                
                if level_key not in pos['tp_triggered'] and pnl_pct >= level_pct:
                    self._close_partial(symbol, pos, OPTIMIZED_CONFIG['DYNAMIC_SCALPING_CLOSE_PCT'], price_to_check, current_time, level_key)
                    pos['tp_triggered'].add(level_key)
    ...
    def _look_for_entries(self, current_time, current_prices, data_map):
        ...
        for symbol, df in data_map.items():
            ...
            # Use optimized signals
            long_ok, _ = EntrySignalsOptimized.check_signals(df_slice, "LONG")
            if long_ok:
                candidates.append({
                    'symbol': symbol,
                    'direction': 'LONG',
            ...
            short_ok, _ = EntrySignalsOptimized.check_signals(df_slice, "SHORT")
            if short_ok:
                candidates.append({
                    'symbol': symbol,
                    'direction': 'SHORT',
        ...
        if candidates:
            ...
            for candidate in candidates[:slots_available]:
                self._open_position(candidate['symbol'], candidate['direction'], candidate['row'], current_time)
    
    def _open_position(self, symbol, direction, row, entry_time):
        entry_price = row['close']
        atr = row['ATR']
        
        sl = ATRManagerOptimized.calculate_initial_stop(entry_price, atr, direction)
    ...
    def _generate_report(self):
        ...
        csv_filename = f"/Users/laurazapata/Desktop/BACKTEST_OPTIMIZED_30DAYS.csv"
...
def main():
    ...
    backtester = OptimizedBacktester(initial_balance=10000)

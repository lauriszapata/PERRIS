"""Backtest 1 Posición - Abril 2025"""
import pandas as pd
import numpy as np
...
class SinglePositionBacktester:
    ...
    def is_trading_hours(self, timestamp):
        ...
        if hour < SUPER_OPTIMIZED_CONFIG['TRADING_START_HOUR']: return False
        if hour > SUPER_OPTIMIZED_CONFIG['TRADING_END_HOUR']: return False
        if hour == SUPER_OPTIMIZED_CONFIG['TRADING_END_HOUR'] and minute >= SUPER_OPTIMIZED_CONFIG['TRADING_END_MINUTE']: return False
        return True
    
    def run_backtest(self, data_map, start_date, end_date):
        print(f"\n{'='*80}\n🔬 1 POSICIÓN - ABRIL 2025\n{'='*80}\n")
    ...
    def _monitor_positions(self, current_time, current_prices, data_map):
        for symbol in list(self.open_positions.keys()):
            ...
            current_price, current_high, current_low, current_atr = current_row['close'], current_row['high'], current_row['low'], current_row['ATR']
            ...
            if not pos.get('breakeven_triggered', False) and pnl_pct >= Config.BREAKEVEN_TRIGGER_PCT:
                if pos['direction'] == 'LONG': pos['sl'] = max(pos['sl'], pos['entry_price'] * 1.001)
    ...
    def _check_tp(self, symbol, pos, current_price, current_high, current_low, current_time):
        price_to_check = current_high if pos['direction'] == 'LONG' else current_low
        pnl_pct = (price_to_check - pos['entry_price']) / pos['entry_price'] if pos['direction'] == 'LONG' else (pos['entry_price'] - price_to_check) / pos['entry_price']
        
        for level in SUPER_OPTIMIZED_CONFIG['TP_LEVELS']:
            level_key = level['name']
            if level_key not in pos['tp_triggered'] and pnl_pct >= level['pct']:
    ...
    def _close_partial(self, symbol, pos, close_pct, exit_price, exit_time, reason):
        ...
        if size_to_close < 0.01: return
        pnl = (exit_price - pos['entry_price']) * size_to_close if pos['direction'] == 'LONG' else (pos['entry_price'] - exit_price) * size_to_close
        entry_value, exit_value = pos['entry_price'] * size_to_close, exit_price * size_to_close
    ...
    def _close_position(self, symbol, exit_price, exit_time, reason):
        ...
        entry_value, exit_value = pos['entry_price'] * pos['current_size'], exit_price * pos['current_size']
        commission = (entry_value + exit_value) * self.commission_rate
        net_pnl = pnl - commission
    ...
    def _look_for_entries(self, current_time, current_prices, data_map):
        ...
        for symbol, df in data_map.items():
            ...
            if symbol in self.symbol_cooldowns:
                ...
                if current_time < cooldown_end: continue
            ...
            if long_ok: candidates.append({'symbol': symbol, 'direction': 'LONG', 'row': current_row, 'score': current_row['ADX']})
            short_ok, _ = EntrySignalsSuperOptimized.check_signals(df_slice, "SHORT")
            if short_ok: candidates.append({'symbol': symbol, 'direction': 'SHORT', 'row': current_row, 'score': current_row['ADX']})
    ...
    def _record_equity(self, current_time, current_prices):
        ...
        for symbol, pos in self.open_positions.items():
            if symbol in current_prices:
                ...
                unrealized = (current_price - pos['entry_price']) * pos['current_size'] if pos['direction'] == 'LONG' else (pos['entry_price'] - current_price) * pos['current_size']
    ...
    def _generate_report(self):
        ...
        csv_filename = f"/Users/laurazapata/Desktop/BACKTEST_1POS_ABRIL2025.csv"

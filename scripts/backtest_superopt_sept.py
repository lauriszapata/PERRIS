"""
Backtest SUPER-OPTIMIZADO - Septiembre 2025
Blacklist expandida (10 símbolos) + SL 4.0 ATR
"""
...
# EXPANDED Blacklist (10 símbolos - peores performers)
SYMBOL_BLACKLIST_EXPANDED = [
    # Original 5
    "POL/USDT", "NEAR/USDT", "APT/USDT", "TRX/USDT", "LINK/USDT",
    # Nuevos 5 (peores en backtest optimizado)
...
class EntrySignalsSuperOptimized:
    ...
    def check_signals(df, direction):
        ...
        try:
            ...
            adx_val = last['ADX']
            results['ADX'] = {'status': adx_val >= SUPER_OPTIMIZED_CONFIG['ADX_MIN'], 'value': f"{adx_val:.2f}"}
            ...
            if direction == "LONG":
                results['MACD'] = {'status': macd_line > macd_signal, 'value': f"L:{macd_line:.4f}"}
            else:
                results['MACD'] = {'status': macd_line < macd_signal, 'value': f"L:{macd_line:.4f}"}
            
            vol = last['volume']
            vol_avg = last['Vol_SMA20']
            vol_min = SUPER_OPTIMIZED_CONFIG['VOLUME_MIN_MULTIPLIER']
            results['Volume'] = {'status': vol >= vol_min * vol_avg, 'value': f"{vol:.2f}"}
            
            atr_val = last['ATR']
            close_val = last['close']
            volatility_pct = atr_val / close_val
            vol_max = SUPER_OPTIMIZED_CONFIG['VOLATILITY_MAX']
...
class SuperOptimizedBacktester:
    ...
    def run_backtest(self, data_map, start_date, end_date):
        print(f"\n{'='*80}")
        print(f"🔬 BACKTEST SUPER-OPTIMIZADO: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        print(f"{'='*80}")
        print(f"Horario: 5:00 AM - 11:50 AM (ESTRICTO)")
        print(f"SL: {SUPER_OPTIMIZED_CONFIG['SL_ATR_MULTIPLIER']}x ATR (INCREMENTADO)")
        print(f"Exposure: ${SUPER_OPTIMIZED_CONFIG['FIXED_EXPOSURE_USD']}")
        print(f"TP Levels: 2 fijos + dinámicos cada 1%")
        print(f"Filtros: ADX≥25, Vol≥1.3x, Volatility<2.5%")
        print(f"Blacklist EXPANDIDA: 10 símbolos excluidos")
        print(f"Símbolos: {len(SUPER_OPTIMIZED_SYMBOLS)}")
        print(f"{'='*80}\n")
        ...
        for symbol in list(self.open_positions.keys()):
            ...
            if pos['direction'] == 'LONG':
                if current_high > pos['highest_price']:
                    ...
                    new_sl = ATRManagerSuperOptimized.calculate_trailing_stop(
    ...
    def _close_partial(self, symbol, pos, close_pct, exit_price, exit_time, reason):
        ...
        self.closed_trades.append({
            ...
            'size': size_to_close, 'pnl': pnl, 'commission': commission,
            'net_pnl': net_pnl, 'exit_reason': f"TP_{reason}", 'partial': True
        })
    ...
    def _close_position(self, symbol, exit_price, exit_time, reason):
        ...
        self.closed_trades.append({
            ...
            'entry_time': pos['entry_time'], 'exit_time': exit_time,
            'entry_price': pos['entry_price'], 'exit_price': exit_price,
            'size': pos['current_size'], 'pnl': pnl, 'commission': commission,
            'net_pnl': net_pnl, 'exit_reason': reason, 'partial': False
        })
    ...
    def _look_for_entries(self, current_time, current_prices, data_map):
        ...
        for symbol, df in data_map.items():
            ...
            if long_ok:
                candidates.append({'symbol': symbol, 'direction': 'LONG', 'row': current_row})
            
            short_ok, _ = EntrySignalsSuperOptimized.check_signals(df_slice, "SHORT")
            if short_ok:
                candidates.append({'symbol': symbol, 'direction': 'SHORT', 'row': current_row})
    ...
    def _record_equity(self, current_time, current_prices):
        ...
        self.equity_curve.append({
            'timestamp': current_time, 'equity': total_equity,
            'balance': self.balance, 'open_positions': len(self.open_positions)
        })
    
    def _generate_report(self):
        print(f"\n{'='*80}")
        print(f"📊 RESULTADOS SUPER-OPTIMIZADOS (SEPTIEMBRE 2025)")
        print(f"{'='*80}\n")
        ...
        csv_filename = f"/Users/laurazapata/Desktop/BACKTEST_SUPEROPT_SEPT2025.csv"
        df_trades.to_csv(csv_filename, index=False)
        print(f"\n💾 Trade log: {csv_filename}")
        print(f"\n{'='*80}\n")
        
        return {
            'total_pnl': total_pnl, 'final_balance': self.balance,
...
def main():
    ...
    loader = DataLoader()
    print("📡 Downloading data for September 2025...")
    ...
    backtester = SuperOptimizedBacktester(initial_balance=10000)

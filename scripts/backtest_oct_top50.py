"""
Backtest Octubre 2025 - Top 50 Símbolos - 1 Posición - 24/7 - Criterios Extremos
"""
import pandas as pd
import numpy as np
...
# CONFIGURATION
BACKTEST_CONFIG = {
    'TRADING_START_HOUR': 0, 'TRADING_END_HOUR': 23, 'TRADING_END_MINUTE': 59, # 24/7
...
# Top 50 Liquid Symbols (Approximate list)
TOP_50_CANDIDATES = [
    ...
    "TIA/USDT", "PEPE/USDT", "WLD/USDT", "FET/USDT", "SEI/USDT" # Extras to ensure we have enough after blacklist
]
...
class ATRManager:
    ...
    def calculate_initial_stop(entry_price, atr_entry, direction):
        multiplier = BACKTEST_CONFIG['SL_ATR_MULTIPLIER']
        if direction == "LONG":
    ...
    def calculate_trailing_stop(current_sl, extreme_price, current_atr, direction, entry_price):
        multiplier = BACKTEST_CONFIG['TRAILING_SL_ATR_MULTIPLIER']
        if direction == "LONG":
...
class EntrySignalsExtreme:
    ...
    def check_signals(df, direction):
        ...
        try:
            ...
            # 2. ADX (Extreme: >= 25)
            adx_val = last['ADX']
            results['ADX'] = {'status': adx_val >= BACKTEST_CONFIG['ADX_MIN']}
            
            # 3. RSI (Standard)
            rsi_val = last['RSI']
            if direction == "LONG": results['RSI'] = {'status': rsi_val > 35}
            else: results['RSI'] = {'status': 30 < rsi_val < 55}
            
            # 4. MACD (Standard)
            macd_line = last['MACD_line']
            macd_signal = last['MACD_signal']
            if direction == "LONG": results['MACD'] = {'status': macd_line > macd_signal}
            else: results['MACD'] = {'status': macd_line < macd_signal}
            
            # 5. Volume (Extreme: >= 1.3x Avg)
            vol = last['volume']
            vol_avg = last['Vol_SMA20']
            results['Volume'] = {'status': vol >= BACKTEST_CONFIG['VOLUME_MIN_MULTIPLIER'] * vol_avg}
            
            # 6. Volatility (Extreme: < 2.5%)
            atr_val = last['ATR']
            close_val = last['close']
            volatility_pct = atr_val / close_val
            results['Volatility'] = {'status': volatility_pct < BACKTEST_CONFIG['VOLATILITY_MAX']}
            
            # 7. MTF Trend (Optional - skipped for speed/simplicity in this script, or assume True)
            results['MTF_Trend'] = {'status': True, 'optional': True}
            
            # 8. Structure (Optional)
            results['Structure'] = {'status': True, 'optional': True}
            
            # Standard Entry Decision
            standard_entry = all(r['status'] for k, r in results.items() if not r.get('optional', False))
            
            # Early Entry Logic (Fast Trend + Indicators)
            if not standard_entry:
                ema8 = last['EMA8']
                ema20 = last['EMA20']
                fast_trend_ok = (ema8 > ema20) if direction == "LONG" else (ema8 < ema20)
                
                # For Early Entry, we still enforce the Extreme Indicators
...
class Backtester:
    def __init__(self, initial_balance=10000):
        ...
        self.max_open_symbols = 1 # 1 Position Max
    ...
    def run_backtest(self, data_map, start_date, end_date):
        print(f"\n{'='*80}\n🔬 BACKTEST OCTUBRE 2025 - TOP 50 - 1 POSICIÓN - 24/7\n{'='*80}\n")
        print(f"Símbolos: {len(SYMBOLS)} (Blacklist excluida)")
        print(f"Criterios: Extremos (ADX>={BACKTEST_CONFIG['ADX_MIN']}, Vol>={BACKTEST_CONFIG['VOLUME_MIN_MULTIPLIER']}x)")
    ...
    def _monitor_positions(self, current_time, current_prices, data_map):
        for symbol in list(self.open_positions.keys()):
            ...
            if pos['direction'] == 'LONG' and r['high'] > pos['highest_price']:
                ...
                pos['sl'] = max(pos['sl'], ATRManager.calculate_trailing_stop(pos['sl'], pos['highest_price'], r['ATR'], 'LONG', pos['entry_price']))
            elif pos['direction'] == 'SHORT' and r['low'] < pos['lowest_price']:
                pos['lowest_price'] = r['low']
                pos['sl'] = min(pos['sl'], ATRManager.calculate_trailing_stop(pos['sl'], pos['lowest_price'], r['ATR'], 'SHORT', pos['entry_price']))
    ...
    def _check_tp(self, symbol, pos, cp, ch, cl, ct):
        ...
        for level in BACKTEST_CONFIG['TP_LEVELS']:
        ...
        if pnl_pct >= BACKTEST_CONFIG['DYNAMIC_SCALPING_START']:
            n_levels = int((pnl_pct - BACKTEST_CONFIG['DYNAMIC_SCALPING_START']) / BACKTEST_CONFIG['DYNAMIC_SCALPING_INCREMENT'])
            for n in range(n_levels + 1):
                lpct = BACKTEST_CONFIG['DYNAMIC_SCALPING_START'] + (n * BACKTEST_CONFIG['DYNAMIC_SCALPING_INCREMENT'])
                lkey = f"D{n+1}"
                if lkey not in pos['tp_triggered'] and pnl_pct >= lpct:
                    self._close_partial(symbol, pos, BACKTEST_CONFIG['DYNAMIC_SCALPING_CLOSE_PCT'], price, ct, lkey)
    ...
    def _open_position(self, symbol, direction, row, entry_time):
        ...
        sl = ATRManager.calculate_initial_stop(ep, atr, direction)
    ...
    def _generate_report(self):
        ...
        df.to_csv("/Users/laurazapata/Desktop/BACKTEST_OCT2025_TOP50_1POS.csv", index=False)
        print(f"\n💾 /Users/laurazapata/Desktop/BACKTEST_OCT2025_TOP50_1POS.csv\n{'='*80}\n")

def main():
    start_date = datetime(2025, 10, 1, 0, 0, 0)
    end_date = datetime(2025, 10, 31, 23, 59, 59)
    
    print("\n🚀 Backtest: OCTUBRE 2025 - TOP 50 - 1 POSICIÓN\n")
    loader = DataLoader()

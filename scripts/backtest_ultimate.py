#!/usr/bin/env python3
"""
BACKTEST PROFESIONAL DEFINITIVO
================================
- Sin look-ahead bias
- Comisiones reales (0.05% taker + 0.03% slippage)
- Funding rate (0.01% cada 8h)
- Entrada en OPEN de siguiente vela
- MTF confirmación (1H)
- Filtros estrictos: ADX, RSI, MACD, Volumen, ATR
- Análisis por horario
- Top 10 símbolos más líquidos
"""
...
def get_mtf_trend(df_1h, timestamp):
    """Obtener tendencia MTF para un timestamp dado"""
    # Buscar la vela 1H correspondiente (la más reciente ANTES del timestamp)
...
def check_signal(row, df_1h, config):
    """
    Verificar señales con TODOS los filtros.
    """
    # Validar que tenemos todos los indicadores
    ...
    # Filtro de rango (evitar consolidaciones)
...
def simulate_trade(df, signal_idx, direction, tp_pct, sl_pct, max_candles):
    """Simular trade de forma realista"""
...
def main():
    print("=" * 80)
    print("🔬 BACKTEST PROFESIONAL DEFINITIVO - BUSCANDO 12 MESES POSITIVOS")
    print("=" * 80)
    ...
    hour_options = [
        None,  # Sin filtro
        list(range(8, 22)),  # 8am-10pm UTC
        list(range(12, 20)),  # 12pm-8pm UTC (hora pico)
        list(range(0, 8)) + list(range(20, 24)),  # Noche/madrugada
    ]
    
    # Combinaciones de símbolos
    symbol_combos = [
        SYMBOLS,  # Todos
        ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'XRPUSDT', 'BNBUSDT'],  # Top 5
        ['BTCUSDT', 'ETHUSDT'],  # Solo mayores
        ['SOLUSDT', 'XRPUSDT', 'DOGEUSDT', 'ADAUSDT'],  # Alts populares
        ['AVAXUSDT', 'LINKUSDT', 'DOTUSDT'],  # Alts medianas
    ]
    
    best_configs = []
    total_combos = (len(tp_options) * len(sl_options) * len(adx_options) * 
                   len(vol_options) * len(direction_options) * len(cooldown_options) *
                   len(hour_options) * len(symbol_combos))
    
    print(f"   Total combinaciones: {total_combos}")
    
    tested = 0
    
    for tp, sl, adx, vol, direction, cooldown, hours, symbols_use in product(
        tp_options, sl_options, adx_options, vol_options, 
        direction_options, cooldown_options, hour_options, symbol_combos
    ):
        tested += 1
        if tested % 500 == 0:
            print(f"   Probando {tested}/{total_combos}... ({tested/total_combos*100:.1f}%)")
    ...
    # Ordenar por meses positivos, luego por PnL

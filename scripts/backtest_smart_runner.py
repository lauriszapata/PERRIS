"""
Backtest "Smart Runner" Strategy - Octubre 2025
Objetivo: Reducir comisiones y dejar correr ganancias.
Estrategia:
1. TP1: Cerrar 50% al +2.5% de ganancia.
2. Runner: El 50% restante corre con Trailing Stop (2.0 ATR).
3. Breakeven: Al +1.0%.
"""
import pandas as pd
import numpy as np
...
BACKTEST_CONFIG = {
    ...
    'LEVERAGE': 1,
    # SMART RUNNER PARAMS
    'TP1_PCT': 0.025,      # 2.5% Target
    'TP1_CLOSE_PCT': 0.50, # Close 50%
    'BREAKEVEN_PCT': 0.010,# 1.0% Trigger
    # Extreme Criteria (Same as before)
    'ADX_MIN': 25, 
...
class SmartRunnerBacktester:
    ...
    def run_backtest(self, data_map, start_date, end_date):
        print(f"\n{'='*80}\n🔬 BACKTEST SMART RUNNER - OCTUBRE 2025\n{'='*80}\n")
        print(f"Estrategia: TP1 50% @ 2.5% -> Runner Trailing Stop")
    ...
    def _monitor_positions(self, current_time, current_prices, data_map):
        for symbol in list(self.open_positions.keys()):
            ...
            if not pos.get('breakeven_triggered', False) and pnl_pct >= BACKTEST_CONFIG['BREAKEVEN_PCT']:
    ...
    def _check_tp(self, symbol, pos, cp, ch, cl, ct):
        # Only ONE TP level: TP1
        if pos.get('tp1_hit', False): return # Already hit TP1, letting runner run
        ...
        if pnl_pct >= BACKTEST_CONFIG['TP1_PCT']:
            self._close_partial(symbol, pos, BACKTEST_CONFIG['TP1_CLOSE_PCT'], price, ct, "TP1_SECURE")
            pos['tp1_hit'] = True
    ...
    def _close_partial(self, symbol, pos, close_pct, exit_price, exit_time, reason):
        ...
        self.closed_trades.append({'symbol': symbol, 'direction': pos['direction'], 'entry_time': pos['entry_time'], 'exit_time': exit_time, 'entry_price': pos['entry_price'], 'exit_price': exit_price, 'size': size, 'pnl': pnl, 'commission': comm, 'net_pnl': net, 'exit_reason': reason, 'partial': True})
        self.balance += net
        if pos['current_size'] < 0.01: self._close_position(symbol, exit_price, exit_time, f"{reason}_FINAL")
    ...
    def _open_position(self, symbol, direction, row, entry_time):
        ...
        self.open_positions[symbol] = {'symbol': symbol, 'direction': direction, 'entry_price': ep, 'entry_time': entry_time, 'initial_size': size, 'current_size': size, 'sl': sl, 'highest_price': ep if direction == 'LONG' else float('inf'), 'lowest_price': ep if direction == 'SHORT' else float('inf'), 'tp1_hit': False, 'breakeven_triggered': False}
    ...
    def _generate_report(self):
        print(f"\n{'='*80}\n📊 RESULTADOS SMART RUNNER - OCTUBRE 2025\n{'='*80}\n")
        ...
        print(f"   Avg Duration: {(df['exit_time'] - df['entry_time']).dt.total_seconds().mean() / 3600:.1f}h")
        ...
        df.to_csv("/Users/laurazapata/Desktop/BACKTEST_SMART_RUNNER_OCT2025.csv", index=False)
        print(f"\n💾 /Users/laurazapata/Desktop/BACKTEST_SMART_RUNNER_OCT2025.csv\n{'='*80}\n")
...
def main():
    ...
    if data_map:
        backtester = SmartRunnerBacktester(initial_balance=10000)
